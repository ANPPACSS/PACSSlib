...
void CalcSlidePos(string fileNameLYSO, int nSlidePos);
...
int main(int argc, char *argv[])
{
  ...
  switch(argc)
  {
    case 3:
      ...
      break;
    default:
      cout << "Usage: " << argv[0] << " [LYSO ROOT file] [n positions]" << endl;
      return 1;
  }

  CalcSlidePos(fileNameLYSO, nSlidePos);
...
void CalcSlidePos(string fileNameLYSO, int nSlidePos)
{
  ...
  // ROOT stuff
  TFile *rootFile = new TFile(fileNameLYSO.c_str(), "UPDATE");
  ...
  rootFile->Write();
  rootFile->Close();
  cout << "Calculated positions written to " << tName << " in " << fileNameLYSO << endl;
  return;
}
